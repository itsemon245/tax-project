                    ...
                    @forelse ($settings as $for => $settng)
                        ...
                        <div class="tab-pane fade {{ $index === 1 ? 'show active' : '' }}" id="{{ $for }}"
                            ...
                            <div class="container rounded bg-white py-3 px-4">
                                <form action="{{ route('tax.calculate') }}" id="form-{{ $index }}" method="POST"
                                    class="row">
                                    @csrf
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <x-backend.form.text-input label="Phone Number" :value="old('phone') ? auth()->user()->phone : ''" type="number"
                                                name="phone" required>
                                            </x-backend.form.text-input>
                                                ...
                                                @foreach ($services as $key => $service)
                                                    @if ($service)
                                                        @php
                                                            $id = uniqid('service-');
                                                        @endphp
                                                        <label x-data="{ selected: false }"
                                                            :class="selected ? 'check-active' :
                                                                'bg-light'"
                                                            class="form-check-label d-flex gap-2 py-2 px-3 bg-gradient rounded-3"
                                                            for="{{ $id }}">
                                                            <input @change="selected = !selected; console.log(selected)"
                                                                class="form-check-input" type="checkbox" hidden
                                                                id="{{ $id }}" name="services[]"
                                                                value="{{ $service->id }}">
                                                            {{ $service->service }}
                                                        </label>
